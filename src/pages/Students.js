...
function Student() {
    ...
    useEffect(() => {
        const url = 'http://localhost/reactwebsite-app/db.php';//this url is from the db.php file
    ...
    const submitBtn = function (e) {
        ...
        axios({
            ...
            url: 'http://localhost/reactwebsite-app/db.php',   //db link
            data: getData, //data to be transfered
            config: 'Content-Type="multipart/form-data"',
        }).then(function (result) {
            const url = 'http://localhost/reactwebsite-app/db.php';
            axios.get(url).then((response) => {
                setStudents(response.data);
                console.log(students);
            })
    ...
    const upBtn = function (e) {
        ...
        axios({
            method: 'POST',
            url: 'http://localhost/reactwebsite-app/db.php',
            data: getData,
            config: 'Content-Type="multipart/form-data"',
        }).then(function (result) {
            alert("success");
                ...
                <table class="table table-dark table-hover w-100">
                    ...
                    <tbody>{students.map((val) => {
                        return (
                                ...
                                <tr key={val.stud_id}>
                                    <td><input className="form-control mb-3 w-100" placeholder="Enter First Name" defaultValue={val.firstname} id={'fname' + val.stud_id} /></td>
                                    <td><input className="form-control mb-3 w-100" placeholder="Enter First Name" defaultValue={val.lastname} id={'lname' + val.stud_id} /></td>
